...
interface TodayReadingListProps {
  ...
  authors: Author[]
  allTocItems: TocItem[]
  onToggle: (tocItemId: string, roundId: string, checkedAt?: number) => void
...
export function TodayReadingList({ items, authors, allTocItems, onToggle, onSchedule }: TodayReadingListProps) {
  ...
  const [dialogItem, setDialogItem] = useState<{ tocItemId: string; roundId: string } | null>(null)

  const handleDateConfirm = (date: string) => {
    if (!dialogItem) return
    const timestamp = new Date(date + "T12:00:00").getTime()
    onToggle(dialogItem.tocItemId, dialogItem.roundId, timestamp)
  }
