  ...
  if (!author) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20">
        <p className="text-muted-foreground">作者未找到</p>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 border-b border-border px-6 py-6">
        <div className="flex h-16 w-16 items-center justify-center rounded-full border border-border bg-muted text-[28px]">
          {author.name[0]}
        </div>
        <div>
          <h1 className="text-[22px] font-bold text-foreground">{author.name}</h1>
        ...
        <button
          ...
          className="ml-auto rounded-md bg-muted px-3 py-1.5 text-xs font-medium text-foreground hover:bg-accent"
        >
          编辑
        </button>
      </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="py-4">
            <label className="mb-1.5 block text-[13px] font-semibold text-foreground">作者名</label>
