...
drop policy if exists "Users can CRUD own authors" on authors;
create policy "Users can CRUD own authors" on authors for all using (auth.uid() = user_id);
...
drop policy if exists "Users can CRUD own toc_items" on toc_items;
create policy "Users can CRUD own toc_items" on toc_items for all
...
drop policy if exists "Users can CRUD own reading_rounds" on reading_rounds;
create policy "Users can CRUD own reading_rounds" on reading_rounds for all
...
drop policy if exists "Users can CRUD own chapter_statuses" on chapter_statuses;
create policy "Users can CRUD own chapter_statuses" on chapter_statuses for all
  using (exists (
...
drop policy if exists "Users can CRUD own book_tags" on book_tags;
create policy "Users can CRUD own book_tags" on book_tags for all
...
create policy "Anyone can read invite codes" on invite_codes for select to anon, authenticated using (true);

-- cover_images: readable by all
alter table cover_images enable row level security;
drop policy if exists "Anyone can read cover_images" on cover_images;
create policy "Anyone can read cover_images" on cover_images for select using (true);
